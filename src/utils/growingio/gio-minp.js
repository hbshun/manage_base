...
class Uploader {
  ...
  forceFlush() {
    this.uploadTimer &&
      (clearTimeout(this.uploadTimer), (this.uploadTimer = null)),
      this._flush()
  }
  _flush() {
    ;(this.uploadingQueue = this.messageQueue.slice()),
      (this.messageQueue = []),
      this.uploadingQueue.length > 0 &&
        Taro.request({
...
class Page$1 {
  ...
  touch(t) {
    ;(this.path = t.route),
      (this.time = Date.now()),
      (this.query = this.queries[t.route] ? this.queries[t.route] : undefined)
  }
  addQuery(t, e) {
...
function getHandle(t, e, i) {
  ...
  a
    ? Object.keys(a.$slots).forEach(function(t) {
        var s = a.$slots[t]
...
class Observer {
  ...
  get sessionId() {
    return (
      null === this._sessionId && (this._sessionId = Utils.guid()),
      this._sessionId
    )
  }
  resetSessionId() {
  ...
  setUserId(t) {
    ...
    e &&
      100 > e.length &&
      ((this.cs1 = e),
      this.lastPageEvent && this._sendEvent(this.lastPageEvent))
  }
  clearUserId() {
    this.cs1 = null
  }
  collectImp(t, e = null) {
    ...
    this._observer && this._observer.disconnect(),
      (this._observer = t.isComponent
        ? t.createIntersectionObserver({ observeAll: true })
        : Taro.createIntersectionObserver(t, { observeAll: true })),
      (this._relative = e
      ...
      this._relative.observe('.growing_collect_imp', t => {
        t.id &&
          !i[t.id] &&
          (this.track(
            t.dataset.gioTrack && t.dataset.gioTrack.name,
            t.dataset.gioTrack && t.dataset.gioTrack.properties
          ),
          (i[t.id] = true))
      })
  }
  appListener(t, e, i) {
    this.isPauseSession ||
      ...
      'onShow' == e
        ? (this._parseScene(i),
          !this.lastCloseTime ||
          Date.now() - this.lastCloseTime > this.keepAlive ||
          (this.lastScene && this.scene !== this.lastScene)
            ? (this.resetSessionId(),
              this.sendVisitEvent(i),
              (this.lastVstArgs = i),
              (this.lastPageEvent = undefined))
        ...
        ? ((this.lastScene = this.scene),
          ...
          this.isPauseSession ||
            ((this.lastCloseTime = Date.now()), this.sendVisitCloseEvent()))
        : 'onError' == e && this.sendErrorEvent(i))
  }
  pageListener(t, e, i) {
    if (
      (this.growingio.debug &&
        console.log('Page.', t.route, '#', e, Date.now()),
      'onShow' === e)
    )
      this.isPauseSession
        ? (this.isPauseSession = false)
      ...
      var n = null,
        s = null
      2 > i.length
        ? 1 === i.length && (i[0].from ? (n = i[0]) : i[0].title && (s = i[0]))
        : ((n = i[0]), (s = i[1])),
        this.pauseSession(),
  ...
  actionListener(t, e) {
    if (
      'handleProxy' === e &&
      this.growingio.vueRootVMs &&
      this.growingio.vueRootVMs[this.currentPage.path]
    ) {
      let i = new VueProxy(
        this.growingio.vueRootVMs[this.currentPage.path]
      ).getHandle(t)
    ...
    this.growingio.taroRootVMs &&
      this.growingio.taroRootVMs[e] &&
      (e = this.growingio.taroRootVMs[e]),
      this.growingio.debug && console.log('Click on ', e, Date.now()),
      'tap' === t.type || 'longpress' === t.type
        ? this.sendClick(t, e)
  ...
  identify(t, e) {
    undefined !== t &&
      0 !== t.length &&
      (this.growingio.login(t),
      this._sendEvent({ t: 'vstr', var: { openid: t, unionid: e } }))
  }
  setVisitor(t) {
  ...
  sendVisitEvent(t) {
    ...
    if (
      (this.growingio.appVer && (i.cv = this.growingio.appVer + ''),
      t.length > 0)
    ) {
      var n = t[0]
      ;(i.p = n.path),
  ...
  sendPage(t) {
    ...
    this.lastPageEvent
      ? ((e.rp = this.lastPageEvent.p),
        this.useLastPageTime &&
          ((e.tm = this.lastPageEvent.tm), (this.useLastPageTime = false)))
      : (e.rp = this.scene ? `scn:${this.scene}` : null),
  ...
  sendPageShare(t, e, i) {
    this._sendEvent({
      ...
      n: 'onShareAppMessage',
      var: {
        from: e ? e.from : undefined,
        target: e && e.target ? e.target.id : undefined,
        title: i ? i.title : undefined,
        path: i ? i.path : undefined
      }
    })
  }
  sendClick(t, e) {
    ...
    n.dataset.title
      ? (s.v = n.dataset.title)
      : n.dataset.src && (s.h = n.dataset.src),
      undefined !== n.dataset.index &&
        (s.idx = /^[\d]+$/.test(n.dataset.index)
  ...
  sendChange(t, e) {
    var i = {
      ...
      s = { x: `${n.id}#${e}` }
    if (
      -1 !== ['blur', 'change', 'confirm'].indexOf(t.type) &&
      n.dataset.growingTrack
    ) {
      if (!t.detail.value || 0 === t.detail.value.length) return
      'string' == typeof t.detail.value
        ? (s.v = t.detail.value)
        : '[object Array]' === Object.prototype.toString.call(t.detail.value) &&
          (s.v = t.detail.value.join(','))
    }
    ;('change' === t.type &&
      t.detail &&
      t.detail.source &&
      'autoplay' === t.detail.source) ||
      ((i.e = [s]), this._sendEvent(i))
  }
  sendTabClick(t) {
    var e = {
      t: 'clck',
...
class Weixin {
  constructor(t) {
    ;(this._location = null),
      ...
      (this._uid = Taro.getStorageSync('_growing_uid_')),
      this._uid && 36 !== this._uid.length && (t.forceLogin = false),
      (this._esid = Taro.getStorageSync('_growing_esid_'))
  }
  get location() {
  ...
  get systemInfo() {
    return (
      null == this._systemInfo && (this._systemInfo = Taro.getSystemInfoSync()),
      this._systemInfo
    )
  }
  set esid(t) {
    ;(this._esid = t), Taro.setStorageSync('_growing_esid_', this._esid)
  }
  get esid() {
    return this._esid || (this._esid = 1), this._esid
  }
  set uid(t) {
    ;(this._uid = t), Taro.setStorageSync('_growing_uid_', this._uid)
  }
  get uid() {
    return this._uid || (this.uid = Utils.guid()), this._uid
  }
  syncStorage() {
    Taro.getStorageSync('_growing_uid_') ||
      Taro.setStorageSync('_growing_uid_', this._uid)
  }
  requestLocation() {
  ...
  getNetworkType() {
    return new Promise(t => {
      Taro.getNetworkType({ success: e => t(e), fail: () => t(null) })
    })
  }
  getPageTitle(t) {
    var e = ''
    try {
      if (
        (t.data.title &&
          t.data.title.length > 0 &&
          (e = Array.isArray(t.data.title)
            ? t.data.title.join(' ')
            : t.data.title),
        0 === e.length && __wxConfig)
      ) {
        if (__wxConfig.tabBar) {
          var i = __wxConfig.tabBar.list.find(
            e => e.pathPath == t.route || e.pagePath == `${t.route}.html`
          )
          i && i.text && (e = i.text)
        }
        if (0 == e.length) {
          ...
          e = n
            ? n.window.navigationBarTitleText
  ...
  _getSetting() {
    return new Promise(t => {
      Taro.getSetting({ success: t, fail: t })
    })
  }
  _getLocation() {
    return new Promise(t => {
...
var VdsInstrumentAgent = {
  ...
  hook: function(t, e) {
    return function() {
      var i,
        n = arguments ? arguments[0] : undefined
      if (
        n &&
        n.currentTarget &&
        -1 != VdsInstrumentAgent.actionEventTypes.indexOf(n.type)
      )
        try {
      ...
      if (
        (this._growing_app_ && 'onShow' !== t
          ? (i = e.apply(this, arguments))
          : this._growing_page_ &&
            -1 === ['onShow', 'onLoad', 'onTabItemTap', 'onHide'].indexOf(t) &&
            (i = e.apply(this, arguments)),
      ...
      if (
        this._growing_page_ &&
        -1 !== VdsInstrumentAgent.pageHandlers.indexOf(t)
      ) {
        ...
        if (-1 !== ['onShow', 'onLoad', 'onTabItemTap', 'onHide'].indexOf(t))
          i = e.apply(this, arguments)
        else {
  ...
  hookComponent: function(t, e) {
    return function() {
      ...
      if (
        i &&
        i.currentTarget &&
        -1 != VdsInstrumentAgent.actionEventTypes.indexOf(i.type)
      )
        try {
  ...
  instrument: function(t) {
    ...
    return (
      t._growing_app_ &&
        VdsInstrumentAgent.appHandlers.map(function(e) {
          t[e] || (t[e] = VdsInstrumentAgent.defaultAppCallbacks[e])
        }),
      t._growing_page_ &&
        VdsInstrumentAgent.pageHandlers.map(function(e) {
          t[e] ||
            'onShareAppMessage' === e ||
            (t[e] = VdsInstrumentAgent.defaultPageCallbacks[e])
        }),
      t
    )
  },
  instrumentTaroPageComponent: function(t) {
    if (t.methods) {
      ...
      for (let i in e)
        if (
          'function' == typeof e[i] &&
          -1 != VdsInstrumentAgent.pageHandlers.indexOf(i)
        ) {
          const n = e[i]
          t.methods[i] = function() {
            return (
              VdsInstrumentAgent.observer.pageListener(this, i, arguments),
  ...
  instrumentComponent: function(t) {
    if (t.methods) {
      ...
      for (let i in e)
        'function' == typeof e[i] &&
          (t.methods[i] = this.hookComponent(i, e[i]))
    }
    return t
  },
  GrowingPage: function(t) {
    return (
      (t._growing_page_ = true),
      VdsInstrumentAgent.originalPage(VdsInstrumentAgent.instrument(t))
    )
  },
  GrowingComponent: function(t) {
    return VdsInstrumentAgent.originalComponent(
      VdsInstrumentAgent.instrumentComponent(t)
    )
  },
  GrowingBehavior: function(t) {
    return VdsInstrumentAgent.originalBehavior(
      VdsInstrumentAgent.instrumentComponent(t)
    )
  },
  GrowingApp: function(t) {
    return (
      (t._growing_app_ = true),
      VdsInstrumentAgent.originalApp(VdsInstrumentAgent.instrument(t))
    )
  },
...
class GrowingIO {
  ...
  init(t, e, i = {}) {
    ;(this.projectId = t),
      ...
      (this.forceLogin = i.forceLogin || false),
      (this.followShare = i.followShare || false),
      (this.usePlugin = i.usePlugin || false),
      (this.getLocation = i.getLocation || false),
      ...
      i.host &&
        i.host.indexOf('http') >= 0 &&
        (this.host = 'https://' + i.host.slice(i.host.indexOf('://') + 3)),
  ...
  login(t) {
    if (this.forceLogin)
      for (var e of ((this.weixin.uid = t),
  ...
  _proxyTaro(t) {
    ...
    t.createComponent = function(t, n) {
      ...
      for (; s && s.prototype; ) {
        const i = Object.keys(
          Object.getOwnPropertyDescriptors(s.prototype) || {}
        )
        for (let n = 0; i.length > n; n++)
          if (i[n].startsWith('func__')) {
            ...
            e.taroRootVMs[i[n]] =
              r +
              '_' +
              ('' + t.prototype[i[n]]).match(
                /this\.__triggerPropsFn\(\"(.+)\",/
              )[1] +
              '_' +
              o
          }
      ...
      t.mixin({
        created: function() {
          ...
          for (let e of Object.keys(this))
            0 > t.indexOf(e) ||
              ...
              (Object.defineProperty(this[e], 'proxiedName', { value: e }),
              Object.defineProperty(this[e], 'isProxied', { value: true }))
        },
        beforeMount: function() {
          ...
          t.$mp &&
            'page' === t.$mp.mpType &&
            t.$mp.page &&
            (e.vueRootVMs[t.$mp.page.route] = t)
        }
      })
    }
  }
...
var growingio = new GrowingIO(),
  gio = function() {
    ...
    if (t) {
      ...
      if (e.length < 2)
        console.log(
          "初始化 GrowingIO SDK 失败。请使用 gio('init', '你的GrowingIO项目ID', '你的微信APP_ID', options);"
        )
