...
const chooseDiscounts = {
  ...
  clickConfirmDiscount: function() {
    let orderDataObj = this.data.orderDataObj
    let couponsArr = []
    const coupons = this.data.showCoupons
    const that = this

    if (coupons.a_coupon) {
      orderDataObj.a_coupon = coupons.a_coupon.used_amount
    }

    if (coupons.c_coupon) {
      orderDataObj.c_coupon = coupons.c_coupon.used_amount
    }

    if (coupons.coupons) {
      orderDataObj.gift_coupons = ''
      ...
      orderDataObj.gift_coupons = couponsArr.join(',')
    }
    // orderDataObj.gift_coupons = '3485,430953';
    orderDataObj.products = this.data.orderDataObj.products
