...
const pageData = {
  ...
  getOrderInfo: function() {
    const products = this.data.orderDataObj.products
    const that = this
    ...
    util.requestLoginUrl({
      url: constant.getScreenOrderInfo + '?products=' + products,
      method: 'GET',
  ...
  handleOrderInfo: function(data) {
    ...
    const otherInfo = {}
    let shopLocation = ''
    if (data.brands[0].shop && data.brands[0].shop.floor) {
      shopLocation =
        data.brands[0].shop.floor.name +
        data.brands[0].shop.location +
        data.brands[0].shop.name
    }
  ...
  createOrder: function() {
    ...
    util.requestLoginUrl({
      url: constant.getScreenOrderInfo,
      method: 'POST',
      data: orderDataObj,
  ...
  postOrder: function(orderId) {
    const gotoUrl =
      '/electronicScreen/order/detail?id=' +
      orderId +
      '&type=screen&from_share=1'
      ...
      <View className="order-page">
        ...
        <View className="shipments">
          <View className="choose-shipments" onClick={this.showExpress}>
            ...
            <View className="right help-youself">
              {'到' + shopLocation + '专柜自提'}
            </View>
          </View>
